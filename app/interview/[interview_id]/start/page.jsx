...
function StartInterview() {
  ...
  useEffect(() => {
    ...
    // Prevent duplicate Vapi instances
    if (vapi.current) {
      return;
    }
  ...
  const GenerateFeedback = async () => {
    try {
      ...
      // If Content is an object (e.g., error response), convert or handle it
      if (typeof Content !== "string") {
        if (Content.error) {
          const errorMsg =
            typeof Content.error === "string"
              ? Content.error
              : JSON.stringify(Content.error) || "Unknown error";
      ...
      // Remove starting ```json\n and ending ```, also remove <think>...</think> tags from reasoning models
      let FINAL_CONTENT = Content.replace(/<think>[\s\S]*?<\/think>/gi, "") // Remove thinking tags from reasoning models
        .replace(/^```json\s*/i, "") // remove starting ```json + optional whitespace
        .replace(/\s*```$/, "")
        .trim();
      ...
      try {
        ...
        toast.error(
          "Failed to parse feedback. The AI response was not valid JSON.",
        );
        return;
      }
